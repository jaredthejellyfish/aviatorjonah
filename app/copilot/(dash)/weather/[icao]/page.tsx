...
export async function generateMetadata({
	...
	return {
		title: `Copilot | Weather at ${icao}`,
		description: `Weather information for ${icao}`,
	};
}
...
export default async function WeatherComponent({ params }: Props) {
	const user = await currentUser();

	const { icao } = await params;

	if (!user || !icao || icao.length !== 4) {
		return redirect("/copilot/dashboard/weather");
	}
	...
	const { weather, homeAirport } = await getWeatherData(userSettings, icao);
					...
					<div className="space-y-4">
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div className="bg-slate-700/50 backdrop-blur-sm border border-slate-600/50 rounded-lg p-4 relative">
								...
								<p className="text-lg font-medium text-slate-200 pr-8">
									{metarData?.wdir
